...
<p:declare-step type="px:dtbook-to-ssml" version="1.0"
	...
	<p:option name="separate-skippable" required="false"
		select="'true'" />

	<p:import
		href="http://www.daisy.org/pipeline/modules/common-utils/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/text-to-ssml/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/tts-helpers/library.xpl" />

	<p:variable name="dc-lang"
		select="//meta[@name='dc:Language']/@content" />
	...
	<px:get-tts-annotations
		content-type="application/x-dtbook+xml" name="get-config-annot">
		<p:input port="config">
			<p:pipe port="config" step="main" />
		</p:input>
	</px:get-tts-annotations>
	...
	<p:choose name="get-annotations">
		...
		<p:otherwise>
			<p:output port="result" primary="true" sequence="true" />
			<p:load
				href="http://www.daisy.org/pipeline/modules/text-to-ssml/dtbook-annotating.xsl" />
		</p:otherwise>
	</p:choose>
	...
	<px:text-to-ssml name="ssml-gen">
		...
		<p:with-option name="lang"
			select="if ($dc-lang) then $dc-lang else 'en'" />
	</px:text-to-ssml>

	<px:message message="End SSML generation for DTBook" />
	<p:sink />
