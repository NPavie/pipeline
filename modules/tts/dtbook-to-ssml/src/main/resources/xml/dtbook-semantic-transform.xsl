...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	xmlns:pho="http://braillenet.org/phonetic" exclude-result-prefixes="#all" version="2.0">
	...
	<!-- Braillet dtbook phonetic annotation 
		Insert a ssml:phoneme when annotations pho:ipa or pho:sampa are found 
		in a w tag -->
			...
			<ssml:phoneme>
				<xsl:choose>
					...
					<xsl:when test="@pho:sampa">
						...
						<xsl:attribute name="ssml:ph">
							<xsl:value-of select="@pho:sampa"/>
						</xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="ssml:alphabet" select="'ipa'"/>
						<xsl:attribute name="ssml:ph">
							<xsl:value-of select="@pho:ipa"/>
						</xsl:attribute>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:apply-templates select="node()"/>
			...
			<xsl:choose>
				...
				dt:is-numeral(dt:normalize(dt:title-prefix($level/preceding-sibling::*[dt:is-level(.)][1])))">
									<ssml:say-as interpret-as="cardinal">
										...
										<!-- alternatively, it could be converted to an Arabic numeral -->
									</ssml:say-as>
								</xsl:when>
								<xsl:otherwise>
									<xsl:apply-templates select="node()"/>
