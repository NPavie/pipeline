...
<xsl:stylesheet
	...
	<!-- Create a new TTS sentence which contains the @alt attribute of the 
		images. -->
	...
	<!-- Test to insert phoneme when a span with phonetic attributes is found  
	<xsl:template match="dt:span[@pho:ipa]|span[@pho:sampa]">
		<dt:w>
	...
	<!-- Those elements can't have <w> in them according to the spec, so we 
		add the <w> here -->
	...
	<!-- Wrap links with words in order to make sure they can be matched against 
		lexicon entries. -->
	<!-- (provided that DTBook links cannot contain words, for old versions 
		of the DTBook spec) -->
	...
	<!-- Detect Roman numerals. While it would be useful to make this code available 
		for other formats than DTBook (i.e. XHTML and ZedAi), the algorithm to iterate 
		over the titles of the same level can be quite different from one format 
		to another, XHTML being less strict than DTBook regarding headers' positioning. -->
	<xsl:variable name="headers"
		select="('levelhd', 'hd', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6')" />
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:variable name="header"
						select="ancestor::*[$headers = local-name()][1]" />
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:variable name="level"
								select="$header/ancestor::*[dt:is-level(.)][1]" />
							<xsl:choose>
								<xsl:when
									test="($text = 'i' and
	...
	<xsl:function name="dt:normalize">
		...
		<xsl:analyze-string
			select="lower-case(string-join($nodes, ''))" regex="^(.+)[).]?$">
	...
	<xsl:function name="dt:title-prefix">
		...
		<xsl:value-of
			select="(($level//*[$headers = local-name()])[1]//dt:w)[1]/text()" />
	</xsl:function>
	...
	<xsl:function name="dt:is-level" as="xs:boolean">
		<xsl:param name="node" />
		<xsl:value-of
			select="starts-with(local-name($node), 'level')" />
	</xsl:function>
	...
	<xsl:function name="dt:is-numeral" as="xs:boolean">
		...
		<xsl:value-of
			select="matches($normalized-word, '^[ivxclm]+$')" />
	</xsl:function>
