...
<p:declare-step version="1.0" name="main"
	type="px:tts-for-dtbook" xmlns:p="http://www.w3.org/ns/xproc"
	...
	<p:input port="fileset.in">
		<p:documentation xmlns="http://www.w3.org/1999/xhtml">
			<p>Input fileset including DTBook documents, lexicons and CSS
				stylesheets.</p>
		</p:documentation>
	</p:input>
	...
	<p:input port="config">
		<p:documentation xmlns="http://www.w3.org/1999/xhtml">
			<p>Configuration file with lexicons, voices declaration and various
				properties.</p>
		</p:documentation>
	</p:input>
	...
	<p:output port="content.out" primary="true" sequence="true">
		...
		<p:documentation xmlns="http://www.w3.org/1999/xhtml">
			<p>Copy of the DTBook documents enriched with ids, words and
				sentences, and without inlined aural CSS.
			</p>
		</p:documentation>
	</p:output>
	...
	<p:option name="audio" required="false" px:type="boolean"
		select="'true'">
		<p:documentation xmlns="http://www.w3.org/1999/xhtml">
			<h2 px:role="name">Enable Text-To-Speech</h2>
			<p px:role="desc">Whether to use a speech synthesizer to produce audio
				files.</p>
		</p:documentation>
	</p:option>
	...
	<p:import
		href="http://www.daisy.org/pipeline/modules/ssml-to-audio/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/dtbook-to-ssml/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/dtbook-break-detection/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/daisy3-utils/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/tts-helpers/library.xpl" />
	<p:import
		href="http://www.daisy.org/pipeline/modules/css-speech/library.xpl" />

	<!-- Find the sentences and the words, even if the Text-To-Speech is off. -->
	<p:for-each name="lexing">
		<p:iteration-source>
			<!-- For now, the for-each is actually not needed since there is only 
				one DTBook. -->
		...
		<px:isolate-daisy3-skippable
			name="isolate-skippable">
			<p:input port="sentence-ids">
				<p:pipe port="sentence-ids" step="break" />
			</p:input>
			<p:with-option name="id-prefix"
				select="concat('i', p:iteration-position())" />
		</px:isolate-daisy3-skippable>
	</p:for-each>
			...
			<p:for-each name="for-each.content">
				...
				<p:split-sequence name="sentence-ids">
					...
					<p:with-option name="test"
						select="concat('position()=', p:iteration-position())" />
				</p:split-sequence>
