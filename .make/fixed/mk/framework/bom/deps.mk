framework/bom/VERSION := 1.15.5-SNAPSHOT
...
$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/framework-bom/1.15.5-SNAPSHOT/framework-bom-1.15.5-SNAPSHOT.pom : framework/bom/.install.pom | .group-eval
	+$(EVAL) if (new File("$@").exists()) touch("$@"); else exit(1);

$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/framework-bom/1.15.5-SNAPSHOT/framework-bom-1.15.5-SNAPSHOT% : framework/bom/.install% | .group-eval
	+$(EVAL) if (new File("$@").exists()) touch("$@"); else exit(1);

.SECONDARY : framework/bom/.install.pom
framework/bom/.install.pom : | .maven-init .group-eval
	+$(EVAL) mvn.installPom("$(TARGET_DIR)/optimized/framework/bom");

framework/bom/.install.pom : %/.install.pom : $(TARGET_DIR)/optimized/%/pom.xml %/.compile-dependencies | %/.test-dependencies
...
$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/framework-bom/1.15.5/framework-bom-1.15.5.% \
$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/framework-bom/1.15.5/framework-bom-1.15.5-% : framework/bom/.release
	+//
