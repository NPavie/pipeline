...
$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/assembly/1.15.4/assembly-1.15.4.pom : assembly/.install.pom | .group-eval
	+$(EVAL) if (new File("$@").exists()) touch("$@"); else exit(1);

$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/assembly/1.15.4/assembly-1.15.4% : assembly/.install% | .group-eval
	+$(EVAL) if (new File("$@").exists()) touch("$@"); else exit(1);
